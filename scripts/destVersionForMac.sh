...
download_wechat() {
    ...
    if [ -z "$DOWNLOAD_LINK" ]; then
        echo_color "red" "Failed to get download link from website!"
        clean_data 1
    fi
    
    print_separator
    echo_color "yellow" "Downloading the newest WeChatMac..."
    echo_color "green" "Download URL: $DOWNLOAD_LINK"
    print_separator
    ...
    echo_color "green" "Download completed successfully!"
}
...
get_version() {
    ...
    echo_color "green" "Version: $VERSION"
}
...
create_release() {
    ...
    RELEASE_BODY=$(cat "WeChatMac/$VERSION/WeChatMac-$VERSION.dmg.sha256")
    
    gh release create "v$VERSION_TAG" \
        "WeChatMac/$VERSION/WeChatMac-$VERSION.dmg" \
        -F "WeChatMac/$VERSION/WeChatMac-$VERSION.dmg.sha256" \
        -t "WeChat For Mac v$VERSION_TAG"
...
main() {
    ...
    if [ "$NOW_SUM256" = "$LATEST_SUM256" ] && [ -n "$LATEST_SUM256" ]; then
        echo_color "green" "This version already exists in releases (same SHA256)!"
        clean_data 0
    fi
